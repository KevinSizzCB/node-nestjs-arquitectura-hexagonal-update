...
describe('Pruebas al controlador de usuarios', () => {
  ...
  beforeAll(async () => {
    ...
    daoUsuario = createStubObj<DaoUsuario>(['listar', 'perfil'], sinonSandbox);
  ...
  it('debería listar el usuario por id', () => {

    const usuarios: any[] = [{ nombre: 'Lorem ipsum', fechaCreacion: (new Date().toISOString()) }];
    daoUsuario.perfil.returns(Promise.resolve(usuarios));
